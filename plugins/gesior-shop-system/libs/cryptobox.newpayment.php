...
*/
	...
	global $db;
	$query = $db->query("select processed FROM `crypto_payments` WHERE paymentID = ".intval($paymentID)." AND processed = 1 LIMIT 1");
	//if (!$processed && $payment_details["confirmed"])
	if ($query->rowCount() == 0 && $payment_details["confirmed"])
	{
		$points = false;
		foreach($config['cryptobox']['options'] as $cost => $_points) {
			if($payment_details["amountusd"] * 100 == $cost * 100) {
				$points = $_points;
			}
		}

		if($points === false) {
			log_append('cryptobox.log', 'Invalid payment cost: ' . $cost . '. Transaction details: ' . print_r($payment_details, true));
			die('Invalid cost.');
		}
